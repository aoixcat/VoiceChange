...
class CycleGAN(object):
    ...
    def build_model(self):
        ...
        self.generation_B = self.generator(inputs = self.input_A_real, reuse = False, scope_name = 'generator_A2B', num_features = self.num_features)
        self.cycle_A = self.generator(inputs = self.generation_B, reuse = False, scope_name = 'generator_B2A', num_features = self.num_features)

        self.generation_A = self.generator(inputs = self.input_B_real, reuse = True, scope_name = 'generator_B2A', num_features = self.num_features)
        self.cycle_B = self.generator(inputs = self.generation_A, reuse = True, scope_name = 'generator_A2B', num_features = self.num_features)

        self.generation_A_identity = self.generator(inputs = self.input_A_real, reuse = True, scope_name = 'generator_B2A', num_features = self.num_features)
        self.generation_B_identity = self.generator(inputs = self.input_B_real, reuse = True, scope_name = 'generator_A2B', num_features = self.num_features)
